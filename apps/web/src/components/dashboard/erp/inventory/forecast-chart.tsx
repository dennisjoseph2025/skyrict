...
function stockoutDateLabel(date: string | null): {
    text: string;
    color: string;
} {
    if (!date)
    ...
    if (days <= 0)
        return {
            text: "Imminent",
            color: "bg-red-600/10 text-red-800 ring-red-600/30",
        };
    if (days <= 14)
        return {
            text: `${days}d`,
            color: "bg-red-500/10 text-red-700 ring-red-500/30",
        };
    if (days <= 30)
        return {
            text: `${days}d`,
...
export function ForecastChart() {
    ...
    useEffect(() => {
        ...
        getCatalogProducts()
            ...
            .catch(() => {
                if (!cancelled)
                    setProductsError("Could not load the product catalog.");
            });
        return () => {
            cancelled = true;
        };
    ...
    return (
        <div className="space-y-6">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="pt-1">
                                        <Badge
                                            variant="outline"
                                            className={`text-xs ring-1 ${so.color}`}
                                        >
                                            Stockout: {so.text}
                                        </Badge>
                                    </div>
            ...
            {status.state === "idle" && !productsError && (
                <p className="text-sm text-muted-foreground">
                    Select a product above to view its demand forecast and
                    stockout risk.
                </p>
            )}
        </div>
