...
export function DataQualityClient() {
        ...
        if (listResult.status === "rejected") {
            ...
            if (error instanceof ApiError && error.status === 403) {
                ...
                listError =
                    error instanceof ApiError
                        ? error.message
                        : "Could not load the employee table.";
            }
        } else {
    ...
    const columns: ErpColumn<EmployeeQualityScore>[] = [
        ...
        },
        {
            ...
            render: (row) => (
                <span className="tabular-nums">
                    {percent(row.mandatoryScore, 0.5)}
                </span>
            ),
        },
        {
        ...
        },
        {
            ...
            render: (row) => (
                <span className="tabular-nums">
                    {percent(row.documentScore, 0.25)}
                </span>
            ),
        },
        {
                ...
                >
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="rounded-xl border border-border bg-card p-4">
                            ...
                            <p className="mt-2 text-2xl font-semibold tabular-nums">
                                {(kpi.gradeDistribution["D"] ?? 0) +
                                    (kpi.gradeDistribution["F"] ?? 0)}{" "}
