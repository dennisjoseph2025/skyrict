...
**The one idea to internalize: stock is a ledger, not a number.** You never "set the count to 50". You record a movement (`+50 received`) and the count is _derived_ from the movements. This makes stock verifiable against money records.
...
| `models/`       | ORM mapping of §3 tables (one file per table)           | Anything else             |
